  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const listBody = {
          con: JSON.stringify({ mode: "getDateFrameList" }),
          p: "{}",
          f: "DynamicReport (get getDateOption data)",
        };
        const listResponse = await CallApi(listBody);
        setAllDateOptions(listResponse?.rd || []);

        // STEP 3️⃣: Fetch existing report-wise selection settings
        const settingBody = {
          con: JSON.stringify({ mode: "getDateFrameSettingsReportWise" }),
          p: JSON.stringify({ ReportId: location.state.ReportId }),
          f: "DynamicReport (get Setting Date data)",
        };
        const settingResponse = await CallApi(settingBody);

        if (settingResponse?.rd?.length > 0) {
          const activeIds = settingResponse.rd
